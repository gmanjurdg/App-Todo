...
export const Employeelist = () => {
    const { employees, removeEmployee,addEmployee, editEmployee } = useContext(GlobalContext);
    useEffect(() => {
                ...
                {employees.map(employee => (
                    <div className="flex items-center bg-gray-100 mb-10 shadow" key={employee.id}>
                        <div className="flex-auto text-left px-4 py-2 m-2">
                            <p className="text-gray-900 leading-none">Name: {employee.name}</p>
                            <p className="text-gray-600">User Name: {employee.username}</p>
                            <span className="inline-block text-sm font-semibold mt-1">Email Id: {employee.email}</span>
                        </div>
